...
function ProfileCard({ profile }) {
  ...
  return (
    <button type="button" className="pc-card-wrapper pc-card-button profile-card" onClick={handleClick}>
      <article className="pc-card">
        <div className="pc-cover">
          {bannerUrl ? (
            <img src={bannerUrl} alt={publicName} className="pc-cover-img" />
          ) : (
            <div className="pc-cover-fallback">{initials}</div>
          )}
        </div>
        <div className="pc-avatar-container">
          {avatarUrl ? (
            <img src={avatarUrl} alt={publicName} className="pc-creator-avatar" />
          ) : (
        ...
        <div className="pc-footer">
          <div className="pc-footer-left">
            {/* Avatar oculto aquí, se muestra en pc-avatar-container */}
            {avatarUrl ? (
              <img src={avatarUrl} alt={publicName} className="pc-creator-avatar" />
            ) : (
              <div className="pc-creator-avatar pc-creator-fallback">{initials}</div>
            ...
            <div className="pc-meta">
              <h3 className="pc-title">{publicName}</h3>
